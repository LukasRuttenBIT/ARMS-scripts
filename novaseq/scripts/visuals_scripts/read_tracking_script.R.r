...
# ---- SETTINGS ----------------------------------------------------------------
NOVASEQ_DIR <- "novaseq/COI"  # folder with track_Batch*.txt files
...
track <- read_tracking(NOVASEQ_DIR, "NovaSeq")

# Sort batches sensibly (Run1 < Run2 … < Batch1 < Batch2 …)
track$batch <- gsub("_mod4$", "", track$batch)

batch_order <- paste0(
  "Batch",
  sort(as.integer(sub("Batch", "", grep("^Batch", unique(track$batch), value = TRUE))))
)
